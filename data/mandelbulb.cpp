...
JuliaSetDE(vec3 P, vec3 *Color)
{
    ...
    for (int I = 0; I < 10; ++I)
    {
        ...
        MinOrbitDist = Min(MinOrbitDist, Length(Z));
...
inline f32
SceneDE(vec3 P, vec3 *Color)
{
    //return JuliaSetDE(P - Vec3(0.0f, 1.2f, 0.0f), Color);
    return MandelbulbDE(P - Vec3(0.0f, 1.2f, 0.0f), Color);
}

inline f32
SceneDE(vec3 P)
{
    return SceneDE(P - Vec3(0.0f, 1.2f, 0.0f), 0);
}
            ...
            for (int SampleIndex = 0; SampleIndex < SampleCount; ++SampleIndex)
            {
                ...
                for (int BounceIndex = 0; BounceIndex < 8; ++BounceIndex)
                {
                    ...
                    for (int I = 0; I < 512 && T < TMax; ++I)
                    {
                        ...
                        f32 Scale = 2.0f;
                        f32 D = Scale*SceneDE(P/Scale, &ObjectColor);
                        if (D < 0.0001f)
                        {
                            Id = 0;
                            break;
                    ...
                    if (Id == 0)
                    {
                        ...
                        Ro = Ro + T*Rd;
                        Rd = Normalize(N + 0.99f*Vec3(RandomBilateral(), RandomBilateral(), RandomBilateral()));
                    }
                    else
                    {
                        Color = Atten * Mat;
                ...
                FinalColor += SampleContrib * Color;
            }
